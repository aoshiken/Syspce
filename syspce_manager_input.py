...
try:
    ...
    g_use_volatility = True
except:
    print("Missing Volatility dependency, going on without Volatility support...")
    g_use_volatility = False
...
class InputManager(Manager_):
    ...
    def __init__(self, data_buffer_in, data_condition_in):
        ...
        Manager_.__init__(self, data_buffer_in, data_condition_in)
    ...
    def _process_messages(self, message_list):
        for message in message_list:
            ...
            if message._subtype == MessageSubType.TERMINATE:
                ...
                self._read_eventlog(message._src, message._content[0])

            elif g_use_volatility and message._subtype == MessageSubType.READ_FROM_MEMDUMP:
